    ...
    if (acceptLabel && acceptCheckbox) {
        acceptLabel.addEventListener("click", (event) => {
            if (acceptCheckbox.disabled) {
                ...
                if (errorElem) {
                    errorElem.textContent = "Najpierw wybierz wszystkie dania na stronie";
                    errorElem.style.display = "block";
                    setTimeout(() => {
                        errorElem.style.display = "none";
                    }, 3000);
                } else {
                    alert("Najpierw wybierz wszystkie dania na stronie");
                }
            }
        });
    }
...
const n = (e, i = null) => {
  ...
  new AirDatepicker(e, {
    locale: {
      ...
      daysShort: ["Nd", "Pn", "Wt", "Śr", "Czw", "Pt", "So"],
      months: ["Styczeń", "Luty", "Marzec", "Kwiecień", "Maj", "Czerwiec", "Lipiec", "Sierpień", "Wrzesień", "Październik", "Listopad", "Grudzień"],
      monthsShort: ["Sty", "Lut", "Mar", "Kwi", "Maj", "Cze", "Lip", "Sie", "Wrz", "Paź", "Lis", "Gru"]
    },
    // Минимальная дата для выбора
    minDate: minDate,
    // Отключаем выбор дат, которые меньше minDate или попадают на выходные (сб, вс)
...
<script>
  ...
  class App {
    ...
    updateCsrfToken = (newToken) => {
      const csrfMeta = document.querySelector('meta[name="csrf-token"]');
      const csrfInput = document.querySelector('input[name="csrf_token"]'); // Добавляем скрытое поле
    ...
    getOrderDetails = () => {
      ...
      // Функция для извлечения всех значений menu_options_id
      ...
      return {
        ...
        building: building, // Значение из поля "klatka"
    ...
    sendOrder = async () => {
      await this.refreshCSRFToken(); // Обновляем CSRF-токен перед отправкой
      ...
      try {
        ...
        if (result.new_csrf_token) {
          this.updateCsrfToken(result.new_csrf_token); // Обновляем CSRF-токен на клиенте
        }
