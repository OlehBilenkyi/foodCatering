...
// Логування помилок у файл (робимо це ДО запуску сесії)
...
if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
    header('Location: /admin/admin.php'); // Перенаправляємо на сторінку авторизації, якщо користувач не авторизований
    exit();
}
...
// Перевіряємо, що з'єднання з базою даних встановлено
if (!$pdo) {
    die("Помилка підключення до бази даних.");
}

// Перевірка відправлення форми для додавання нового замовлення
$notificationMessage = "";
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $customerFullname = isset($_POST['customer_fullname']) ? trim($_POST['customer_fullname']) : '';
    ...
    $customerStreet = isset($_POST['customer_street']) ? trim($_POST['customer_street']) : '';
    $customerHouseNumber = isset($_POST['customer_house_number']) ? trim($_POST['customer_house_number']) : '';
    $customerApartment = isset($_POST['customer_apartment']) ? trim($_POST['customer_apartment']) : '';
    $customerFloor = isset($_POST['customer_floor']) ? trim($_POST['customer_floor']) : '';
    $customerGateCode = isset($_POST['customer_gate_code']) ? trim($_POST['customer_gate_code']) : '';
    $customerKlatka = isset($_POST['customer_klatka']) ? trim($_POST['customer_klatka']) : ''; // Нове поле Klatka
    $customerNotes = isset($_POST['customer_notes']) ? trim($_POST['customer_notes']) : '';
    $totalPrice = isset($_POST['total_price']) ? trim($_POST['total_price']) : '';
    $status = isset($_POST['status']) ? trim($_POST['status']) : 'pending'; // Додаємо статус з форми
    $packages = isset($_POST['packages']) ? $_POST['packages'] : [];

    if ($customerFullname !== '' && $customerEmail !== '' && $customerPhone !== '' && $totalPrice !== '' && !empty($packages)) {
        try {
            // Вставляємо замовлення в таблицю orders
            ...
            $stmt->execute([
                ':customer_fullname' => htmlspecialchars($customerFullname ?? '', ENT_QUOTES, 'UTF-8'),
                ...
                ':customer_street' => htmlspecialchars($customerStreet ?? '', ENT_QUOTES, 'UTF-8'),
                ':customer_house_number' => htmlspecialchars($customerHouseNumber ?? '', ENT_QUOTES, 'UTF-8'),
                ':customer_apartment' => htmlspecialchars($customerApartment ?? '', ENT_QUOTES, 'UTF-8'),
                ':customer_floor' => htmlspecialchars($customerFloor ?? '', ENT_QUOTES, 'UTF-8'),
                ':customer_gate_code' => htmlspecialchars($customerGateCode ?? '', ENT_QUOTES, 'UTF-8'),
                ':customer_klatka' => htmlspecialchars($customerKlatka ?? '', ENT_QUOTES, 'UTF-8'), // Додаємо значення поля Klatka
                ':customer_notes' => htmlspecialchars($customerNotes ?? '', ENT_QUOTES, 'UTF-8'),
                ':total_price' => $totalPrice,
                ':status' => htmlspecialchars($status ?? 'pending', ENT_QUOTES, 'UTF-8') // Додаємо значення статусу
            ]);

            $orderId = $pdo->lastInsertId();
            error_log("Новий заказ додано успішно з ID: " . $orderId);

            // Вставляємо інформацію про кожен пакет в таблицю order_packages
            foreach ($packages as $index => $package) {
                error_log("Обробляємо пакет №" . ($index + 1) . ": " . print_r($package, true));

                // Преобразуем строку дат доставки в массив
                if (isset($package['delivery_dates']) && is_string($package['delivery_dates'])) {
                    $package['delivery_dates'] = explode(',', $package['delivery_dates']);
                }

                if (
                    isset($package['calories'], $package['quantity'], $package['delivery_dates']) &&
                    is_numeric($package['calories']) &&
                    is_numeric($package['quantity']) &&
                    is_array($package['delivery_dates']) &&
                    !empty($package['delivery_dates'])
                ) {
                    $stmt = $pdo->prepare("INSERT INTO order_packages (order_id, calories, quantity) VALUES (:order_id, :calories, :quantity)");
                    $stmt->execute([
                        ':order_id' => $orderId,
                        ':calories' => intval($package['calories']),
                        ':quantity' => intval($package['quantity'])
                    ]);

                    $orderPackageId = $pdo->lastInsertId();
                    error_log("Пакет додано успішно з ID: " . $orderPackageId);

                    // Вставляємо дати доставки в таблицю delivery_dates для кожного пакета
                    foreach ($package['delivery_dates'] as $date) {
                        if (!empty($date)) {
                            $stmtDate = $pdo->prepare("INSERT INTO delivery_dates (order_package_id, delivery_date) VALUES (:order_package_id, :delivery_date)");
                            $stmtDate->execute([
                                ':order_package_id' => $orderPackageId,
                                ':delivery_date' => trim($date)
                            ]);
                            error_log("Дата доставки додана успішно для пакету ID: " . $orderPackageId . ", дата: " . $date);
                        } else {
                            error_log("Попередження: дата доставки пуста для пакету ID: " . $orderPackageId);
                        }
                    }
                } else {
                    error_log("Помилка: дані пакету некоректні або відсутні. Пакет №" . ($index + 1) . ": " . print_r($package, true));
                }
            }

            $notificationMessage = "Новий заказ успішно додано.";
        } catch (PDOException $e) {
            error_log('Помилка при додаванні замовлення: ' . $e->getMessage());
            $notificationMessage = 'Помилка при додаванні замовлення: ' . $e->getMessage();
        }
    } else {
        $notificationMessage = 'Будь ласка, заповніть всі обов’язкові поля.';
    }
}
?>
