<?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/config/db.php'; // Оновлений шлях до файлу бази даних
require_once $_SERVER['DOCUMENT_ROOT'] . '/admin/functions.php'; // Підключаємо функції

if (session_status() === PHP_SESSION_NONE) {
    session_start(); // Стартуємо сесію для використання CSRF токену
}

// Увімкнення логування помилок для запису у файл
ini_set('log_errors', 1);
ini_set('error_log', $_SERVER['DOCUMENT_ROOT'] . '/logs/error_log.log');
error_reporting(E_ALL); // Логування всіх помилок

// Перевірка, чи авторизований користувач як адміністратор
if (!isset($_SESSION['admin_logged_in']) || $_SESSION['admin_logged_in'] !== true) {
    header('Location: /admin/admin.php'); // Перенаправлення на сторінку входу, якщо користувач не авторизований
    exit();
}

// Генерація CSRF-токена, якщо він ще не встановлений
if (empty($_SESSION['csrf_token'])) {
    $_SESSION['csrf_token'] = bin2hex(random_bytes(32)); // Генеруємо випадковий токен
}

// Перевірка CSRF токена для запобігання CSRF-атак
...
// Перевірка статусу для відображення повідомлень про успіх чи помилки
$successMessage = '';
if (isset($_GET['status'])) {
    $statusMessages = [
        'success' => "Зміни успішно збережені!",
        'week_shifted' => "Порядок тижнів змінено!",
        'deleted' => "Запис успішно видалено!",
        'csrf_error' => "Помилка CSRF-токена. Будь ласка, спробуйте ще раз.",
        'not_found' => "Запис не знайдено або вже видалено.",
        'no_weeks_to_rotate' => "Немає тижнів для переміщення.",
        'rotate_error' => "Помилка при переміщенні тижнів. Спробуйте ще раз.",
        'add_error' => "Помилка при додаванні нового запису. Будь ласка, спробуйте ще раз."
    ];
    if (array_key_exists($_GET['status'], $statusMessages)) {
        $successMessage = $statusMessages[$_GET['status']];
    }
}

// Обробка відправки форми для додавання нового запису
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['week_number'])) {
    ...
    $week_number = htmlspecialchars($_POST['week_number'] ?? '');
    $day_number = htmlspecialchars($_POST['day_number'] ?? '');
    $dish_data = [];

    // Збираємо дані для всіх чотирьох страв
    for ($i = 1; $i <= 4; $i++) {
        $dish_data[$i] = [
            'image' => htmlspecialchars($_POST["dish_{$i}_image"] ?? ''),
            'title' => htmlspecialchars($_POST["dish_{$i}_title"] ?? ''),
            'description' => htmlspecialchars($_POST["dish_{$i}_description"] ?? ''),
            'ingredients' => htmlspecialchars($_POST["dish_{$i}_ingredients"] ?? ''),
            'allergens' => htmlspecialchars($_POST["dish_{$i}_allergens"] ?? ''),
            'energy' => htmlspecialchars($_POST["dish_{$i}_energy"] ?? '0'),
            'fat' => htmlspecialchars($_POST["dish_{$i}_fat"] ?? '0'),
            'carbohydrates' => htmlspecialchars($_POST["dish_{$i}_carbohydrates"] ?? '0'),
            'protein' => htmlspecialchars($_POST["dish_{$i}_protein"] ?? '0'),
            'salt' => htmlspecialchars($_POST["dish_{$i}_salt"] ?? '0'),
            'net_mass' => htmlspecialchars($_POST["dish_{$i}_net_mass"] ?? '0')
        ];
    }

    // Перевірка, що всі дані заповнені
    $valid = true;
    foreach ($dish_data as $dish) {
        if (empty($dish['image']) || empty($dish['title']) || empty($dish['description']) || empty($dish['ingredients']) || empty($dish['allergens'])) {
            $valid = false;
            break;
        }
    }

    if (!empty($week_number) && !empty($day_number) && $valid) {
        try {
            $stmt = $pdo->prepare("INSERT INTO weekly_menu (
    ...
    dish_4_image, dish_4_title, dish_4_description, dish_4_ingredients, dish_4_allergens, dish_4_energy, dish_4_fat, dish_4_carbohydrates, dish_4_protein, dish_4_salt, dish_4_net_mass
            ...
            $values = [$week_number, $day_number];
            for ($i = 1; $i <= 4; $i++) {
                $values = array_merge($values, array_values($dish_data[$i]));
            }

            // Виконуємо запит вставки
            $stmt->execute($values);

            // Перенаправляємо з повідомленням про успішне додавання
            header('Location: /admin/admin_menu.php?status=success');
            exit();
        } catch (Exception $e) {
            error_log("Помилка при додаванні нового запису: " . $e->getMessage());
            header('Location: /admin/admin_menu.php?status=add_error');
            exit();
        }
    }
}

// Код для переміщення поточного тижня в кінець
if (isset($_GET['move_week'])) {
    try {
        $pdo->beginTransaction();

        $stmt = $pdo->prepare("SELECT MAX(week_number) FROM weekly_menu");
        $stmt->execute();
        $maxWeekNumber = $stmt->fetchColumn();

        if ($maxWeekNumber) {
            $stmt = $pdo->prepare("UPDATE weekly_menu SET week_number = :new_week_number WHERE week_number = 1");
            $stmt->execute(['new_week_number' => $maxWeekNumber + 1]);

            $stmt = $pdo->prepare("UPDATE weekly_menu SET week_number = week_number - 1 WHERE week_number > 1");
            $stmt->execute();
        ...
        $pdo->rollBack();
        error_log("Помилка при переміщенні тижня: " . $e->getMessage());
        header('Location: /admin/admin_menu.php?status=rotate_error');
    }
    exit();
}

// Отримання всіх даних для відображення
try {
    $stmt = $pdo->query("SELECT * FROM weekly_menu ORDER BY week_number, day_number");
    $menus = $stmt->fetchAll();
} catch (Exception $e) {
    error_log("Помилка при витяганні даних: " . $e->getMessage());
    $menus = [];
}
?>
...
<head>
    ...
    <style>
        .action-button {
            margin-right: 15px; /* Додаємо відступ між кнопками */
        }
        .compact-buttons .action-button:last-child {
            margin-right: 0; /* Встановлюємо відступ для останньої кнопки на 0 */
        }
        .compact-buttons {
            display: flex;
...
<div class="admin-container mt-5">
    ...
    <div class="card">
        <div class="card-header">
            Управління меню на тиждень
        </div>
        
        <div class="card-body">
            <form method="POST" action="/admin/admin_menu.php" onsubmit="validateForm(event)">
                        ...
                        <div class="card mt-3">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="dish_<?= $i ?>_image" name="dish_<?= $i ?>_image" class="input-control" required>
