...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    for ($i = 1; $i <= 4; $i++) {
        $dish_data[$i] = [
            'image' => htmlspecialchars(trim($_POST["dish_{$i}_image"])),
            'title' => htmlspecialchars(trim($_POST["dish_{$i}_title"])),
            'description' => htmlspecialchars(trim($_POST["dish_{$i}_description"])),
            'ingredients' => htmlspecialchars(trim($_POST["dish_{$i}_ingredients"])),
            'allergens' => htmlspecialchars(trim($_POST["dish_{$i}_allergens"])),
            'energy' => filter_var($_POST["dish_{$i}_energy"], FILTER_VALIDATE_FLOAT),
            'fat' => filter_var($_POST["dish_{$i}_fat"], FILTER_VALIDATE_FLOAT),
            'carbohydrates' => filter_var($_POST["dish_{$i}_carbohydrates"], FILTER_VALIDATE_FLOAT),
            'protein' => filter_var($_POST["dish_{$i}_protein"], FILTER_VALIDATE_FLOAT),
            'salt' => filter_var($_POST["dish_{$i}_salt"], FILTER_VALIDATE_FLOAT),
            'net_mass' => filter_var($_POST["dish_{$i}_net_mass"], FILTER_VALIDATE_FLOAT)
        ];
    }
    ...
    if ($week_number && $day_number && $valid) {
        try {
            // Оновлюємо запис у базі даних
            $stmt = $pdo->prepare("UPDATE weekly_menu SET week_number = ?, day_number = ?, 
            ...
            $stmt->execute([
                ...
                $dish_data[1]['image'], $dish_data[1]['title'], $dish_data[1]['description'], $dish_data[1]['ingredients'], $dish_data[1]['allergens'], $dish_data[1]['energy'], $dish_data[1]['fat'], $dish_data[1]['carbohydrates'], $dish_data[1]['protein'], $dish_data[1]['salt'], $dish_data[1]['net_mass'],
                $dish_data[2]['image'], $dish_data[2]['title'], $dish_data[2]['description'], $dish_data[2]['ingredients'], $dish_data[2]['allergens'], $dish_data[2]['energy'], $dish_data[2]['fat'], $dish_data[2]['carbohydrates'], $dish_data[2]['protein'], $dish_data[2]['salt'], $dish_data[2]['net_mass'],
                $dish_data[3]['image'], $dish_data[3]['title'], $dish_data[3]['description'], $dish_data[3]['ingredients'], $dish_data[3]['allergens'], $dish_data[3]['energy'], $dish_data[3]['fat'], $dish_data[3]['carbohydrates'], $dish_data[3]['protein'], $dish_data[3]['salt'], $dish_data[3]['net_mass'],
                $dish_data[4]['image'], $dish_data[4]['title'], $dish_data[4]['description'], $dish_data[4]['ingredients'], $dish_data[4]['allergens'], $dish_data[4]['energy'], $dish_data[4]['fat'], $dish_data[4]['carbohydrates'], $dish_data[4]['protein'], $dish_data[4]['salt'], $dish_data[4]['net_mass'],
                $id
            ]);
...
<div class="container mt-5">
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="edit_menu.php?id=<?= htmlspecialchars($id) ?>">
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="week_number" name="week_number" class="input-control" value="<?= htmlspecialchars($menu['week_number']) ?>" required>
                </div>
                <div class="form-group">
                    <label for="day_number">Номер дня</label>
                    <input type="number" id="day_number" name="day_number" class="input-control" value="<?= htmlspecialchars($menu['day_number']) ?>" required>
                </div>
                        ...
                        <div class="card mt-3">
                            ...
                            <div class="card-body">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="dish_<?= $i ?>_title" name="dish_<?= $i ?>_title" class="input-control" value="<?= htmlspecialchars($menu["dish_{$i}_title"]) ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="dish_<?= $i ?>_image">Назва зображення</label>
                                    <input type="text" id="dish_<?= $i ?>_image" name="dish_<?= $i ?>_image" class="input-control" value="<?= htmlspecialchars($menu["dish_{$i}_image"]) ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="dish_<?= $i ?>_description">Опис страви</label>
                                    <textarea id="dish_<?= $i ?>_description" name="dish_<?= $i ?>_description" class="input-control" rows="3" required><?= htmlspecialchars($menu["dish_{$i}_description"]) ?></textarea>
                                </div>
                                <div class="form-group">
                                    <label for="dish_<?= $i ?>_ingredients">Склад</label>
                                    <textarea id="dish_<?= $i ?>_ingredients" name="dish_<?= $i ?>_ingredients" class="input-control" rows="3" required><?= htmlspecialchars($menu["dish_{$i}_ingredients"]) ?></textarea>
                                </div>
                                <div class="form-group">
                                    <label for="dish_<?= $i ?>_allergens">Алергени</label>
                                    <textarea id="dish_<?= $i ?>_allergens" name="dish_<?= $i ?>_allergens" class="input-control" rows="2" required><?= htmlspecialchars($menu["dish_{$i}_allergens"]) ?></textarea>
                                </div>
                                <div class="form-group">
